...
def main() -> None:
    ...
    if media_type == "video":
        tensor = torch.randn(1, 3, num_frames, image_size, image_size)
    else:
        tensor = torch.randn(1, 3, image_size, image_size)
    if backend == "onnx":
        onnx_path = config.get("export", {}).get("output_path") or "model.onnx"
        outputs = run_onnx_inference(onnx_path, tensor, providers=config.get("inference", {}).get("providers"))
    else:
        ...
        batch = ForgeBatch(x=tensor, media=media_type, task=str(config["task"]), labels={}, paths=[], meta=[])
        outputs = run_torch_inference(model, batch, device=str(config["train"].get("device", "cpu")), data_cfg=data_cfg)
