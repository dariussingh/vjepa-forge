...
"""Reimplements the required mask losses and polygon rasterization utilities for Forge segmentation."""
...
def dice_loss(pred_mask_logits: torch.Tensor, target_masks: torch.Tensor, eps: float = 1.0) -> torch.Tensor:
    """Reimplements DETR/RF-DETR-style dice loss for Forge-native mask supervision."""
...
def sigmoid_ce_mask_loss(pred_mask_logits: torch.Tensor, target_masks: torch.Tensor) -> torch.Tensor:
    """Reimplements sigmoid mask BCE used by DETR-style instance segmentation losses."""
...
def rasterize_polygon(normalized_polygon: list[float], *, output_size: int) -> torch.Tensor:
    """Rasterizes normalized Forge polygons into binary masks for native segmentation losses."""
    ...
    return torch.from_numpy(__import__("numpy").array(mask, dtype="float32"))


def build_instance_targets(label_items: list[dict[str, Any]], *, output_size: int) -> list[dict[str, torch.Tensor]]:
    """Builds Forge-native instance segmentation targets from Forge polygon annotations."""
...
def build_semantic_targets(label_items: list[dict[str, Any]], *, num_classes: int, output_size: int, video_frames: int | None = None) -> torch.Tensor:
    """Builds Forge-native semantic targets by rasterizing Forge polygons into class maps."""
...
def match_instances(
    ...
    """Reimplements Hungarian instance matching for Forge-native mask/class supervision."""
